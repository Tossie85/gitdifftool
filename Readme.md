...
|  0.2.7  |  2026-01-25  | モーダルの表示位置調整                                                                                      |
|  0.2.8  |  2026-01-26  | マージコミット除外設定追加                                                                                  |
|  0.2.9  |  2026-01-27  | マージコミットオプションに変更（すべて、マージコミット以外、マージコミットのみ）                            |
| 0.2.10  |  2026-01-28  | 軽微修正                                                                                                    |
|  0.3.0  |  2026-01-29  | リモートブランチとの差分をチェックできる機能追加<br>レイアウト調整                                          |
|  0.3.1  |  2026-07-26  | バグフィックス（ブランチ取得時の文字コード）                                                                |

## 機能

- gitのローカルで追跡しているブランチ一覧を取得できる
- gitのローカルで取得したコミット一覧を取得できる（0.2.6からユーザ設定で変更可能）
- ２つの任意のブランチ間の差分を取得し、差分ファイルをローカルにコピーできる
- ２つの任意のコミット間の差分を取得し、差分ファイルをローカルにコピーできる
- ローカルでの変更と最新のコミット（HEAD）との差分を取得し、差分ファイルをローカルにコピーできる
- コピー不要なファイルはパスの部分一致で除外できる

## 使い方

- distフォルダの`gitdifftool.exe`をローカルの任意のフォルダに格納
- ダブルクリックで実行（流通していないソフトのためウィルス対策ソフトにチェックされる可能性があります）
  ...
  - pythonが入っている環境であれば、`python3 difftool.py`で起動することもできます
  - `run.bat`に上記のコマンドを入れていますので、`run.bat`から起動することも可能です
...
- 比較モードは以下の３種類を用意しています
  - ブランチ間比較
    - 比較コマンドは`git diff --name-only {branch1} {branch2}`を用いファイル名を取得し、差分一覧を書き出す
    - ２つのブランチ間の差分リストの作成と差分ファイルのローカルへのコピー
    - コピーは`git show {branch}:{filename}`コマンドを使って指定ブランチのファイルを取得します
  - コミット間比較
    - 比較コマンドは`git diff --name-only {commit1} {commit2}`を用いファイル名を取得、差分一覧を書き出す
    - ２つのコミット間の差分リストを用いて差分ファイルのローカルへのコピー
    - コピーは`git show {commit}:{filename}`コマンドを使って指定コミットのファイルを取得します
  - 未コミット比較
...
- 結果は、実行時の時刻フォルダの下に以下を出力します
  - 差分ファイルリスト（テキストファイル）
  - コミットまたはブランチの差分ファイル一式
    - コミットIDまたはブランチ名のフォルダの下にコピーされます
- 差分ファイルのコピーの対象外設定
  - 設定→ワークスペース→コピー対象外設定に改行区切りでパスの一部を指定することで、実行時にコピーをスキップします

## 免責事項

- 本アプリの利用に関するトラブル、不具合について作成者は一切責任を負いません。
- 利用者の自己責任の上でご利用ください。
