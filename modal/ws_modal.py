...
class SelectWorkspaceModal(tk.Toplevel):
    ...
    def _select_workspace(self):
        ...
        if ws_name != "":
            ...
            db.update_or_insert_user_settings(const.US_KEY_CURRENT_WS, ws_name)

            # チェックがあるときはデフォルトワークスペースとしてDBのユーザ設定に登録
            if self.default_ws_selected.get():
                db.update_or_insert_user_settings(const.US_KEY_DEFAULT_WS, ws_name)
            else:
                db.update_or_insert_user_settings(const.US_KEY_DEFAULT_WS, "")
